// Motor de Generación de Certificados PDF - Academia BRISEIN LTDA.
...
// Soporta texto justificado tipográfico, códigos QR vectoriales nítidos, decretos dinámicos y empresas.
...
class CertificatePdfBuilder {
  ...
  /**
   * Dibuja un código QR vectorial de alta definición directamente en el PDF
   */
  /**
   * Dibuja un código QR vectorial de alta definición directamente en el PDF sin dependencias externas
   */
  static async drawQrCode(pdfDoc, page, text, x, y, size) {
    ...
    try {
      ...
      console.error('Error dibujando QR vectorial en PDF:', err);
    }
  }
  ...
  static async buildCertificate(certData) {
    ...
    // 2. Código QR Vectorial en la esquina superior izquierda
    const verificationUrl = certData.verificationUrl || `https://valida.empresasbrisein.com/?c=demo`;
    await this.drawQrCode(pdfDoc, page, verificationUrl, 34, 715, 72);
